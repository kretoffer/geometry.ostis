...
class FormTaskFromCentralizedTestingRecommendationsAgent(ScAgentClassic):
    def __init__(self):
        super().__init__("action_form_task_from_centralized_testing_recommendations")
    
    def on_event(self, event_element: ScAddr, event_edge: ScAddr, action_element: ScAddr) -> ScResult:
        result = self.run(action_element)
        is_successful = result == ScResult.OK
        finish_action_with_status(action_element, is_successful)
        self.logger.info("FormTaskFromCentralizedTestingRecommendationsAgent finished %s",
                         "successfully" if is_successful else "unsuccessfully")
        return result


    def run(self, action_node: ScAddr) -> ScResult:
        self.logger.info("FormTaskFromCentralizedTestingRecommendationsAgent started")

        user, theme = get_action_arguments(action_node, 2)

        all_tests = self.get_all_tasks_from_centralized_testing(theme)
        bad_problems = []
        good_problems = []
        other_problems = []
    ...
    def get_all_tasks_from_centralized_testing(theme: ScAddr) -> list[ScAddr]:
        templ = ScTemplate()
        templ.quintuple(
        ...
        templ.triple(
            ScKeynodes.resolve("concept_task_from_centralized_testing", sc_type.CONST_NODE_CLASS),
            sc_type.VAR_PERM_POS_ARC,
            "_task"
        )
        templ.triple(
    ...
    def delete_previous_task_recommendations_for_user(user: ScAddr) -> bool:
        ...
        templ.quintuple(
            ...
            sc_type.VAR_PERM_POS_ARC,
            ScKeynodes.resolve("nrel_task_from_centralized_testing_recommendations_for_user", sc_type.CONST_NODE_NON_ROLE)
        )
        templ.quintuple(
            user,
